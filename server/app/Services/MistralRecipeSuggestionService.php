...
class MistralRecipeSuggestionService implements RecipeSuggestionProvider
{
    ...
    {
        if (trim($this->apiKey) === '') {
            throw new RuntimeException('Mistral API key is missing. Set MISTRAL_API_KEY in server/.env.');
        }
        ...
        $payload = [
            ...
            'messages' => [
                [
                    'role' => 'system',
                    'content' => 'You generate practical home-cooking recipes from pantry ingredients. Return only valid JSON with the requested fields.',
                ],
                [
                    'role' => 'user',
        ...
        if (!is_array($recipe)) {
            throw new RuntimeException('Mistral returned an invalid recipe response.');
        }
    ...
    private function postJson(string $url, array $payload): array
    {
        ...
        if ($statusCode < 200 || $statusCode >= 300) {
            $message = is_array($decodedResponse)
                ? ($decodedResponse['message'] ?? ($decodedResponse['error']['message'] ?? 'Mistral request failed.'))
        ...
        if (!is_array($decodedResponse)) {
            throw new RuntimeException('Mistral returned an unreadable response.');
        }
    ...
    {
        return json_encode([
            'task' => 'Create one recipe suggestion and return only JSON.',
            ...
            'rules' => (bool) ($preferences['strictMode'] ?? false)
                ? [
                    'Use only availableIngredients.',
                    'Do not add missing ingredients.',
                    'Return missingIngredients as an empty array.',
                ]
                : [
